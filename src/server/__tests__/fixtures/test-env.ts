 ...
 * - Moonshot: Uses API key from ~/.nova-agents/config.json
 */
...
/**
 * Load Moonshot API key from ~/.nova-agents/config.json
 */
function loadMoonshotApiKey(): string | undefined {
  const configPath = join(homedir(), '.nova-agents', 'config.json');

  if (!existsSync(configPath)) {
    console.warn('[test-env] ~/.nova-agents/config.json not found');
    return undefined;
  }
 ...
 * Uses API key from ~/.nova-agents/config.json
