# nova-agents Windows 构建与测试指南

本文档描述 nova-agents Windows 版本的构建流程、发布流程以及测试注意事项。

---

## 概览

nova-agents Windows 版本支持 **x86_64 (64位)** 架构，提供两种分发格式：

| 格式 | 文件 | 用途 |
|------|------|------|
| **NSIS 安装包** | `nova-agents_x.x.x_x64-setup.exe` | 标准安装，有向导界面 |
| **便携版** | `nova-agents_x.x.x_x86_64-portable.zip` | 解压即用，无需安装 |
...
└── releases/
    └── v{VERSION}/
        ├── nova-agents_{VERSION}_x64-setup.exe       # NSIS 安装包
        ├── nova-agents_{VERSION}_x86_64-portable.zip # 便携版
        ├── nova-agents_{VERSION}_x86_64.nsis.zip     # 自动更新包
        └── nova-agents_{VERSION}_x86_64.nsis.zip.sig # 更新签名
```
...
├── nova-agents_x.x.x_x64-setup.exe       # NSIS 安装包
├── nova-agents_x.x.x_x86_64-portable.zip # 便携版
├── nova-agents_x.x.x_x64-setup.nsis.zip  # 自动更新包
└── nova-agents_x.x.x_x64-setup.nsis.zip.sig  # 更新签名
```
...
curl -s https://download.nova-agents.io/update/windows-x86_64.json

# 网站下载 API
curl -s https://download.nova-agents.io/update/latest_win.json
```
...
**注意事项**：
- 覆盖安装后，用户数据（Projects、Providers）应该保留在 `%APPDATA%\nova-agents`
- 如遇到 WebView 缓存问题，可清理 `%LOCALAPPDATA%\nova-agents\EBWebView`
