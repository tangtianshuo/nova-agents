...
**Persistent config edits:** edit `~/.agent-browser/config.json`. **IMPORTANT:** This file is auto-managed by nova-agents — it regenerates on every app startup when `"_managed_by": "nova-agents"` is present. To make persistent edits that survive restarts, you **must** first delete the `"_managed_by"` field, then save your changes. `args` field is split by **both comma and newline** — avoid args containing commas (e.g. `--window-size=W,H` will be split incorrectly; use `--start-maximized` instead). All CLI flags map to camelCase keys (`--executable-path` → `executablePath`).
